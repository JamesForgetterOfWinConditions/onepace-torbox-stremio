...
async function getEpisodes() {
    ...
    try {
        ...
        const sagas = {}; let seasonCounter = 0;
        for (const arc of data.arcs) {
            if (!sagas[arc.saga.name]) { seasonCounter++; sagas[arc.saga.name] = { season: seasonCounter, episodes: [] }; }
            sagas[arc.saga.name].episodes.push(...arc.episodes.map((ep, index) => ({ id: `onepace:${sagas[arc.saga.name].season}:${arc.part + index}`, title: ep.part_name, season: sagas[arc.saga.name].season, episode: arc.part + index, released: new Date(ep.released_at).toISOString(), overview: `Saga: ${arc.saga.name} | Arc: ${arc.name}`, thumbnail: `https://onepace.net/images/episodes/${ep.id}.jpg`, torrents: ep.downloads.reduce((acc, curr) => { if (curr.torrent_url) acc.push(curr.torrent_url); return acc; }, []) })));
        }
        cachedEpisodes = Object.values(sagas).flatMap(saga => saga.episodes); cacheTime = Date.now();
...
builder.defineMetaHandler(async (args) => {
    if (args.id === 'onepace-series') {
        ...
        const meta = { id: 'onepace-series', name: 'One Pace', type: 'series', poster: 'https://i.imgur.com/k91B01C.jpg', background: 'https://i.imgur.com/k91B01C.jpg', logo: 'https://i.imgur.com/v8tT4d9.png', description: 'A fan-edited version of the One Piece anime with filler and padding removed to match the manga’s pacing.', videos: episodes, };
...
builder.defineStreamHandler(async (args) => {
    ...
    try {
        ...
        if (streamUrl) { return Promise.resolve({ streams: [{ title: 'Torbox Stream', url: streamUrl }] }); }
...
async function getTorboxStream(magnet, apiKey) {
    ...
    if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.message || 'Failed to add torrent to Torbox'); }
